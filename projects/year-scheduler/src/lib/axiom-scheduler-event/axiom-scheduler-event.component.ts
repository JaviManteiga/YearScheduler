...
export class AxiomSchedulerEventComponent extends AxiomSchedulerComponentCommon implements OnInit, AfterViewInit {
  ...
  private checkPosition(): void {
    setTimeout(() => {
      const from = moment(this.event.from).diff(moment(this.event.from).startOf('day'), 'minutes');
      const to = moment(this.event.to).diff(moment(this.event.to).startOf('day'), 'minutes');
